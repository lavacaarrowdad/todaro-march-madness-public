...
def matchup_game_for_card(team_a, team_b, live_map, recent_games, locked_games, prefer_top_team=False):
    if not team_a and not team_b:
        return None
    if prefer_top_team and team_a:
        top_game = get_game_for_single_team(team_a, live_map)
        if top_game:
            return top_game
...
def tickets_for(row) -> int:
    return max(
        TICKET_VALUES.get(str(row.get("round_reached", "")).strip(), 0),
        TICKET_VALUES.get(str(row.get("manual_status", "")).strip(), 0),
    )
...
def matchup_info_line(team_a, team_b, live_map, recent_games, locked_games, prefer_top_team=False):
...
def team_line(row, live_map):
    ...
    if str(row.get("espn_team", "")).strip():
        sub.append(f'ESPN: {safe(row["espn_team"])}')
    sub_html = f'<div class="team-sub">{" • ".join(sub)}</div>' if sub else ""
    extra = " money-team" if ticket_ct else ""
    return (
        f'<div class="team-row{extra}" style="background:{person_color(row.get("assigned_name", ""))}">'
        f'<div class="team-main"><span class="seed">{safe(row["seed"])}</span><span class="team-name">{safe(row["team"])}</span>{badge}</div>'
        f'{sub_html}{status_html}{live_line(row, live_map)}</div>'
    )


def matchup_card(top_row, bottom_row, live_map, recent_games, locked_games, title="", tickets=0, prefer_top_team=False):
    title_html = ""
    if title:
...
def render(df, live_map, recent_games, locked_results):
...
def live_bracket_fragment(df: pd.DataFrame):
    ...
    render(df, recent.get("team_map", {}), recent.get("games", []), locked)
